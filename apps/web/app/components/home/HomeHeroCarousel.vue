...
import { mergeHomeHeroSlidesWithFallbacks } from '~/utils/home-hero-content'

const localePath = useLocalePath()
const { locale } = useI18n()
const isEnglish = computed(() => locale.value === 'en-US')
const { data: cmsSlides } = await useAsyncData<HomeHeroSlide[]>(
  () => `home-hero-slides-${locale.value}`,
  () => $fetch<HomeHeroSlide[]>('/api/content/home-hero-slides', { query: { locale: locale.value } }),
  { watch: [locale], default: () => [] },
)
const slides = computed(() => mergeHomeHeroSlidesWithFallbacks(cmsSlides.value, locale.value))
...
const ctaTarget = (value?: string | null) => {
  const target = value?.trim() || '/'
  return /^(https?:\/\/|mailto:|tel:|#)/i.test(target) ? target : localePath(target)
}
...
<template>
  <section class="hero" @mouseenter="paused = true" @mouseleave="paused = false">
    <TransitionGroup name="hero-fade">
      <div :key="activeSlide.documentId || activeSlide.name || activeSlide.title" class="hero-background hero-photo">
        <img
          v-if="activeSlide.backgroundImage?.url"
          :src="activeSlide.backgroundImage.url"
          :alt="activeSlide.backgroundImage.alternativeText || activeSlide.title"
          :style="{ objectPosition: activeSlide.backgroundPosition || 'center center' }"
        />
      </div>
    </TransitionGroup>
    ...
    <div class="site-container hero-content">
      <div class="hero-copy">
        ...
        <Transition name="copy-shift" mode="out-in">
          <div :key="activeSlide.documentId || activeSlide.name || activeSlide.title">
            <h1>{{ activeSlide.title }}</h1>
            <p>{{ activeSlide.description }}</p>
            <div class="hero-actions">
              <NuxtLink v-if="activeSlide.primaryLabel && activeSlide.primaryLink" :to="ctaTarget(activeSlide.primaryLink)" class="primary-action">
                {{ activeSlide.primaryLabel }}<span>↗</span>
              </NuxtLink>
              <NuxtLink v-if="activeSlide.secondaryLabel && activeSlide.secondaryLink" :to="ctaTarget(activeSlide.secondaryLink)" class="secondary-action">
                {{ activeSlide.secondaryLabel }}<span>→</span>
              </NuxtLink>
            </div>
          </div>
        </Transition>
      ...
      <div class="hero-controls">
        <div class="slide-count"><strong>{{ activeNumber }}</strong><span>/ {{ String(slides.length).padStart(2, '0') }}</span></div>
        <div class="progress-track" :style="{ gridTemplateColumns: `repeat(${slides.length}, 1fr)` }">
          <button
            v-for="(slide, index) in slides"
            :key="slide.documentId || slide.name || slide.title"
            type="button"
...
.visual-card span { color: #78adff; font-size: 12px; letter-spacing: .18em; }
...
.visual-card-main { inset: 70px 90px 110px 80px; justify-content: center; padding: 54px; border-color: rgba(120,173,255,.45); background: linear-gradient(145deg, rgba(23,105,224,.34), rgba(255,255,255,.08)); }
...
.platform-core { position: absolute; left: 50%; top: 50%; display: grid; width: 210px; height: 210px; place-items: center; border: 1px solid rgba(120,173,255,.48); border-radius: 50%; background: rgba(255,255,255,.08); box-shadow: 0 0 90px rgba(23,105,224,.2); transform: translate(-50%,-50%); backdrop-filter: blur(12px); }
...
.hero-tag span { width: 32px; height: 2px; background: #5b9cff; }
.hero-copy h1 { margin: 0; max-width: 760px; overflow-wrap: anywhere; white-space: pre-line; font-size: clamp(48px, 5.2vw, 76px); font-weight: 480; letter-spacing: -.055em; line-height: 1.08; text-wrap: balance; }
.hero-copy p { max-width: 650px; margin: 28px 0 0; color: rgba(255,255,255,.77); white-space: pre-line; font-size: 16px; line-height: 1.9; }
.hero-actions { display: flex; flex-wrap: wrap; align-items: center; gap: 26px; margin-top: 38px; }
.primary-action { display: inline-flex; align-items: center; justify-content: space-between; min-width: 174px; padding: 16px 19px; border-radius: 8px; background: var(--accent); font-size: 14px; font-weight: 700; box-shadow: 0 16px 35px rgba(11,77,184,.28); transition: 200ms ease; }
.primary-action:hover { background: #2b7af0; transform: translateY(-2px); }
...
@media (max-width: 767px) {
  ...
  .hero-copy h1 { max-width: 100%; word-break: break-all; font-size: clamp(35px, 9.8vw, 44px); line-height: 1.12; text-wrap: wrap; }
