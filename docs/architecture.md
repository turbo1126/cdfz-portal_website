...
- 样式：`Tailwind CSS 3.4`（由 `@nuxtjs/tailwindcss 6` 集成）
- 状态管理：`Pinia`
- 国际化：`@nuxtjs/i18n`
- CMS：`Strapi 5`
- 数据库：`PostgreSQL 16`
- 包管理：`pnpm`
- 运行环境：Windows Node.js + Docker Desktop（开发），Linux + Docker Compose（生产）
- 反向代理：`Nginx`
...
│  │  ├─ app/
│  │  │  ├─ assets/css/
│  │  │  ├─ components/{layout,home,content}/
│  │  │  ├─ content/fallbacks/
│  │  │  ├─ layouts/
│  │  │  ├─ pages/
│  │  │  ├─ stores/
...
│  │  ├─ server/{api,utils}/
│  │  ├─ public/images/{brand,hero,platforms}/
│  │  └─ nuxt.config.ts
│  └─ cms/
│     ├─ config/
│     ├─ src/api/
...
│  └─ docker/
├─ docs/
├─ references/{company-materials,assets-original}/
├─ compose.yml
├─ compose.prod.yml
├─ pnpm-workspace.yaml
├─ .env.example
...
```mermaid
flowchart LR
    A["浏览器 localhost:3000"] --> B["Nuxt Dev Server on Windows"]
    B --> C["Nuxt Server API / BFF"]
    C --> D["Strapi Dev Server on Windows :1337"]
